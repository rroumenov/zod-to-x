...
namespace GENERICS_INFRA {
    ...
    private:
        bool success;
        std::string message;
        boost::optional<std::unordered_map<std::string, nlohmann::json>> details;
    ...
    public:
        ...
        boost::optional<std::unordered_map<std::string, nlohmann::json>> get_details() const { return this->details; }
        void set_details(boost::optional<std::unordered_map<std::string, nlohmann::json>> value) { this->details = value; }
    };
    ...
    using DiscriminantDataRetrieve = boost::variant<
...
namespace GENERICS_INFRA {
    ...
    inline void to_json(nlohmann::json& j, const DiscriminantDataRetrieve& x) {
        if (x.type() == typeid(HttpSuccessfulResponse<SomeDtoResult>)) {
            j = boost::get<HttpSuccessfulResponse<SomeDtoResult>>(x);
        }
        else if (x.type() == typeid(HttpUnsuccessfulResponse)) {
            j = boost::get<HttpUnsuccessfulResponse>(x);
        }
        else {
